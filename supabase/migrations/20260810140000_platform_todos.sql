/*
  ...
  Sürümler major.minor (1.0, 1.1, 2.0); tek mevcut ve tek hedef sürüm.
*/
...
create table if not exists public.platform_versions (
  ...
  constraint platform_versions_unique_label unique (major, minor)
);
...
create index if not exists platform_versions_order_idx
  on public.platform_versions (major desc, minor desc);

comment on table public.platform_versions is
  'Platform sürümleri (major.minor) — süperadmin yapılacaklar';
...
-- Başlangıç: mevcut 1.0, hedef 1.1
insert into public.platform_versions (major, minor, is_current, is_target, released_at)
select 1, 0, true, false, now()
where not exists (
  select 1 from public.platform_versions where major = 1 and minor = 0
);

insert into public.platform_versions (major, minor, is_current, is_target, released_at)
select 1, 1, false, true, null
where not exists (
  select 1 from public.platform_versions where major = 1 and minor = 1
);
