...
import { verifyWaiterPinAction, type WaiterPinActionState } from "@/app/garson/actions";
...
export default function WaiterPinForm({ slug, nextPath, pinConfigured, businessName }: Props) {
  const baseId = useId();
  const [state, formAction, pending] = useActionState(verifyWaiterPinAction, null as WaiterPinActionState);
  ...
  return (
    ...
    >
      ...
      >
        {pending ? "Doğrulanıyor…" : "Garson paneline geç"}
      </button>
    </form>
  );
}
